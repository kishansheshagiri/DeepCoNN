...
tf.flags.DEFINE_string("valid_data","../data/yelp/penn/kishan/yelp.test", " Data for validation")
tf.flags.DEFINE_string("para_data", "../data/yelp/penn/kishan/yelp.para", "Data parameters")
tf.flags.DEFINE_string("train_data", "../data/yelp/penn/kishan/yelp.train", "Data for training")
tf.flags.DEFINE_string("user_feat", "../data/yelp/penn/kishan/user_attr", "User features")
# ==================================================
...
def train_step(uf_batch, u_batch, i_batch, uid, iid, y_batch, batch_num):
    """
...
def dev_step(uf_batch, u_batch, i_batch, uid, iid, y_batch, writer=None):
    """
...
if __name__ == '__main__':
    ...
    an_pkl_fl = open(FLAGS.user_feat, 'rb')
    temp = pickle.load(an_pkl_fl)
    for key in temp.keys():
        print type(temp[key][0])
        user_f_length = len(temp[key])
        break
    pkl_user = temp
    #pkl_user = {key:int(temp[key]) for key in temp.keys()}    
            ...
            for epoch in range(15):
                ...
                for batch_num in range(ll):
                    ...
                    for i in range(len(uid)):
                        ...
                        uf_batch.append(pkl_user[int(uid[i][0])])
                    #print len(u_batch[0])
                    #print len(uf_batch[0])
                    u_batch = np.array(u_batch)
                    i_batch = np.array(i_batch)
                    uf_batch = np.array(uf_batch)
                    #print u_batch.shape
                    #print uf_batch.shape
                    #print user_f_length

                    t_rmse, t_mae = train_step(uf_batch, u_batch, i_batch, uid, iid, y_batch, batch_num)
                    ...
                    if batch_num % 1000 == 0 and batch_num > 1:
                        ...
                        for batch_num2 in range(ll_test):
                            ...
                            for i in range(len(userid_valid)):
                                ...
                                uf_valid.append(pkl_user[int(userid_valid[i][0])])
                            u_valid = np.array(u_valid)
                            i_valid = np.array(i_valid)
                            uf_valid = np.array(uf_valid)

                            loss, accuracy, mae = dev_step(uf_valid, u_valid, i_valid, userid_valid, itemid_valid, y_valid)
